...
export function useAuth(required = false): [boolean, any, any, string] {
  const auth = useContext(AuthContext);
  if (!auth) {
  ...
  useEffect(() => {
    if (required && !auth.isAuthenticated()) {
      localStorage.setItem("refer_from", location.pathname + location.search);
      navigate("/login");
    }
  }, []);

  return [auth.isAuthenticated(), auth.getHeader(), auth.getCredentials(), auth.getToken()];
}